...
def test_completion_requires_100(data): admin,ra,staff,e=data; add_investigation(e,ra); c=create_capa(ra,**capa_kwargs(e,ra)); pytest.raises(ValidationError,change_capa_status,c,"COMPLETED",ra)
def test_completion_requires_date(data): admin,ra,staff,e=data; add_investigation(e,ra); kw=capa_kwargs(e,ra); kw["completion_percentage"]=100; c=create_capa(ra,**kw); pytest.raises(ValidationError,change_capa_status,c,"COMPLETED",ra)
...
def test_submit_records_user(data): admin,ra,staff,e=data; r=create_report_from_event(e,ra,regulatory_authority="MFDS"); approve_report(r,admin); mark_report_submitted(r,ra); assert r.submitted_by==ra and r.submitted_at
...
def test_docx_version_and_filename(data,tmp_path,settings): admin,ra,staff,e=data; settings.MEDIA_ROOT=tmp_path; r=create_report_from_event(e,ra,regulatory_authority="MFDS"); approve_report(r,admin); p=generate_docx_report(r,ra); assert p.name.endswith("_v1.docx") and r.document_version==1 and p.exists()
