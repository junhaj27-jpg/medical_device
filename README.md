...
이 저장소의 감사 로그, 전자서명, 관리번호 및 파일 격리 기능은 규제 대응을 지원하는 기술적 통제이며 특정 법령이나 전자서명 요건의 충족을 자체적으로 보장하지 않습니다. 조직의 절차, 교육, 권한 검토, 검증 및 규제 판단이 별도로 필요합니다.

배포 순서는 다음과 같습니다.

1. 데이터베이스와 비공개 파일 저장소를 백업합니다.
2. `AUDIT_HMAC_KEY`와 `SIGNATURE_HMAC_KEY`를 KMS 또는 비밀 저장소에서 서로 다른 강한 값으로 주입합니다.
3. `PRIVATE_MEDIA_ROOT`를 웹 루트 밖의 암호화된 저장소로 지정하고 ClamAV 주소를 설정합니다.
4. `python manage.py migrate`로 카운터, 감사 체인, 전자서명 및 첨부파일 보안 필드를 생성합니다.
5. `python manage.py check`와 전체 테스트를 실행합니다.
6. `python manage.py verify_audit_chain`으로 감사 로그 체인을 검증한 뒤 애플리케이션을 기동합니다.

운영 DB는 PostgreSQL을 권장합니다. 연도별 카운터 서비스는 원자적 트랜잭션, 행 잠금과 제한된 재시도를 사용하며 PostgreSQL에서 동시성 통합 테스트를 실행하도록 구성되어 있습니다. SQLite는 개발용이며 운영 동시성 보장을 대신하지 않습니다.

첨부파일은 업로드 후 `QUARANTINED` 상태로 비공개 저장소에 보관되고 `CLEAN` 판정 전에는 다운로드할 수 없습니다. 운영에서는 ClamAV 연결 어댑터와 재시도 큐, 서버 또는 객체 저장소 암호화, 백업 암호화와 파기 작업자를 구성해야 합니다. `INFECTED`와 `SCAN_FAILED` 파일은 일반 사용자에게 제공하지 않습니다.

감사 로그와 전자서명은 HMAC-SHA256 체인을 사용합니다. 정기적으로 `verify_audit_chain`을 실행하고 결과를 별도 보안 저장소에 보관해야 합니다. 보존 만료일, legal hold 및 파기 상태는 DB에서 관리하지만 실제 보존 기간과 파기 승인 절차는 조직 정책에 맞게 운영해야 합니다. HMAC 키 회전 시에는 기존 키 식별자 보존과 재검증 절차가 필요합니다.

전자서명은 승인 직전 비밀번호 재인증, 역할 검사, 자기 승인 금지, 승인 의미와 사유, canonical JSON 및 해시를 기록합니다. 비밀번호 자체는 저장하지 않습니다. 비밀번호 재인증만으로 법적 전자서명 요건이 자동 충족되는 것은 아니며 필요하면 기업 IdP의 MFA 또는 별도 재인증 수단으로 교체해야 합니다.
