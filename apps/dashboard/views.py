...
def dashboard(request):
    ...
    return render(request,"dashboard.html",{"stats":stats,"recent":qs.select_related("medical_device").order_by("-created_at")[:8],"severity_json":json.dumps(severity),"status_json":json.dumps(status)})
@login_required
...
def report_download(request,pk):
    if request.user.role not in {"ADMIN","RA_QA"}: return HttpResponseForbidden()
    event=get_object_or_404(AdverseEvent,pk=pk); path=generate_event_report(event,request.user); AuditLog.objects.create(user=request.user,action="REPORT",model_name="AdverseEvent",object_id=str(pk),object_repr=event.event_number); return HttpResponse(path.read_bytes(),content_type="application/vnd.openxmlformats-officedocument.wordprocessingml.document",headers={"Content-Disposition":f'attachment; filename="{path.name}"'})
@login_required
